var gameObj = {
        ...
        gameOver: function() {
            Response.Write(alert('GAVE OVER!'), location.href = '../html/level3.html');
        },
        //遊戲過關
        winning: function(obj) {
            for (var i = 0; i < 4; i++) {
                for (var j = 0; j < 4; j++) {
                    var obj = this.stage[i][j];
                    if (obj.boxObj.value == 1024) {
                        return true;
                    }
                }
            }
        ...
        win: function() {
            Response.Write(alert('Pass! \n Try next level!'), location.href = '../html/level4.html');
        },
        // 清除重來
        clear: function(x, y) {
            var can = 0;
